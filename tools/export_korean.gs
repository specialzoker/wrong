// ====================================================
// 국어 데이터 JSON 내보내기 스크립트
// 사용법: 국어 앱스크립트 프로젝트에 이 코드를 붙여넣고
...
function exportToJson() {
  ...
  for (var i = 1; i < examData.length; i++) {
    ...
    exams.push({
      ...
      question_pdf_id: extractDriveId(row[5]),  // 국어: F열
      answer_pdf_id:   extractDriveId(row[7])   // 국어: H열
    });
  }
  ...
  for (var i = 1; i < qData.length; i++) {
    ...
    questions.push({
      ...
      score:          qData[i][9],
      is_common:      String(qData[i][10])  // '공통' / '화법과작문' / '언어와매체'
    });
  }

  var output = JSON.stringify({ exams: exams, questions: questions }, null, 2);
  showJsonDialog(output, 'korean.json');
}
